// SupportView – like ResourceView but scoped to employees who have ever been
// planned for a 'support' assignment. Only support chips show up in the
// cells; clicking an empty cell or dragging a chip pre-selects type='support'
// in the AssignmentModal.
const SupportView = ({ s, h }) => {
    ...
    const supportWeeks = timelineWeeks;
    ...
    const safeStart = Math.max(0, Math.min(visibleRange.start, supportWeeks.length - 1));
    const safeEnd = Math.max(safeStart, Math.min(visibleRange.end, supportWeeks.length - 1));
    const visibleWeeks = React.useMemo(
        () => supportWeeks.slice(safeStart, safeEnd + 1),
        [supportWeeks, safeStart, safeEnd]
    );
    const leftSpacerSpan = safeStart;
    const rightSpacerSpan = Math.max(0, supportWeeks.length - 1 - safeEnd);

    return (
        <div className="flex-1 flex flex-col h-full bg-white overflow-hidden">
            <div className="p-4 border-b border-slate-300 bg-gea-50 flex items-center gap-3">
                <h2 className="text-gea-800 text-xl font-semibold shrink-0">{t('support.title')}</h2>
                                    ...
                                    {!isCollapsed && catEmps.map(emp => {
                                        ...
                                        <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            {visibleWeeks.map(w => {
                                                ...
                                                const supportAss = allWeekAss.filter(a => a.type === 'support');
                                                const isOfftime = allWeekAss.some(a => a.type === 'offtime');
                                                return (
                                                    <td key={w.id}
                                                        className={`p-1.5 border-b border-r border-slate-300 relative transition-colors group/cell ${isDeleteMode ? 'bg-rose-50/20' : 'cursor-pointer hover:bg-gea-50/30'} ${isOfftime ? 'bg-slate-50 diagonal-stripes' : 'bg-white'} ${w.id === currentWeek ? 'bg-gea-50/50 border-l border-l-gea-300 border-r-gea-300' : ''} ${w.id < currentWeek ? 'opacity-60' : ''}`}
                                                        onClick={() => { if (!isDeleteMode) { setAssignContext({ empId: emp.id, week: w.id, defaultType: 'support' }); setIsAssignModalOpen(true); } }}
                                                        ...
                                                        <div className={`flex flex-col gap-1 relative z-10 ${compact ? 'min-h-[20px]' : 'min-h-[44px]'}`}>
                                                            {supportAss.map(a => {
                                                                ...
                                                                const dotColor = sc ? sc.dot : 'bg-amber-500';
                                                                ...
                                                                return (
                                                                    <div key={a.id}
                                                                        ...
                                                                        <div className="flex items-center gap-1.5 min-w-0">
                                                                            ...
                                                                            {a.comment && <IconMessageSquare size={11} className="flex-shrink-0 opacity-80"/>}
