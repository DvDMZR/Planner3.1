...
const OfftimeView = ({ s, h }) => {
    ...
    const safeStart = Math.max(0, Math.min(visibleRange.start, offtimeWeeks.length - 1));
    const safeEnd = Math.max(safeStart, Math.min(visibleRange.end, offtimeWeeks.length - 1));
    const visibleWeeks = React.useMemo(
        () => offtimeWeeks.slice(safeStart, safeEnd + 1),
        [offtimeWeeks, safeStart, safeEnd]
    );
    const leftSpacerSpan = safeStart;
    const rightSpacerSpan = Math.max(0, offtimeWeeks.length - 1 - safeEnd);
                                        ...
                                        <tr key={emp.id} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="p-3 border-b border-slate-300 bg-white sticky left-0 z-20 sticky-col-divider">
                                                <div className="flex items-center justify-between gap-2">
                                                    <div className="text-slate-800 font-medium text-sm truncate">{emp.name}</div>
                                                    {emp.vacationDays != null && (() => {
                                                        const used = vacationUsedByEmp.get(emp.id) || 0;
                                                        const total = emp.vacationDays;
                                                        const cls = used > total ? 'bg-rose-50 text-rose-700 border-rose-200'
                                                                  : used === total ? 'bg-amber-50 text-amber-700 border-amber-200'
                                                                  :                  'bg-emerald-50 text-emerald-700 border-emerald-200';
                                                        return (
                                                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full border font-semibold tabular-nums shrink-0 ${cls}`}
                                                                title={t('offtime.vacationTooltip', { year: timelineYear, remaining: Math.max(0, total - used) })}>
                                            ...
                                            {visibleWeeks.map(w => {
                                                ...
                                                return (
                                                    <td key={w.id}
                                                        ...
                                                        onClick={() => { if (!isDeleteMode) { setAssignContext({ empId: emp.id, week: w.id, defaultType: 'offtime' }); setIsAssignModalOpen(true); } }}
                                                        ...
                                                        <div className={`flex flex-col gap-1 relative z-10 ${compact ? 'min-h-[20px]' : 'min-h-[44px]'}`}>
                                                            {offtimeAss.map(a => {
                                                                ...
                                                                return (
                                                                    <div key={a.id}
                                                                        ...
                                                                        onClick={e => { e.stopPropagation(); if (isDeleteMode) { deleteWithUndo(a.id); } else { setAssignContext({ empId: emp.id, week: w.id, existing: a, allowedType: 'offtime' }); setIsAssignModalOpen(true); } }}
                                                                        className={`text-[11px] rounded-md border flex justify-between items-stretch shadow-sm transition-all group/chip overflow-hidden ${isDeleteMode ? 'cursor-pointer hover:bg-rose-50 hover:border-rose-300 hover:text-rose-700 hover:line-through' : 'hover:shadow hover:-translate-y-0.5 cursor-grab active:cursor-grabbing'} bg-slate-100 border-slate-300 text-slate-700 ${pct === 0 ? 'bg-hatched' : ''}`}>
                                                                        <div className="flex items-center gap-1.5 min-w-0">
                                                                            <div className="w-1 flex-shrink-0 self-stretch bg-slate-500"></div>
                                                            ...
                                                            {!compact && offtimeAss.length > 0 && (
                                                                <div
                                                                    onClick={e => { e.stopPropagation(); setAssignContext({ empId: emp.id, week: w.id, defaultType: 'offtime', allowedType: 'offtime' }); setIsAssignModalOpen(true); }}
