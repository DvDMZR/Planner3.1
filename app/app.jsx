...
function App() {
            ...
            try {
                ...
                if (!result.ok) {
                    if (result.reason === 'futureVersion') {
                        showToast(`Diese Datei wurde mit einer neueren App-Version gespeichert (Schema v${result.version}, diese App nutzt v${SCHEMA_VERSION}). Bitte die App aktualisieren, bevor du sie importierst.`, { type: 'error', duration: 10000 });
                    } else {
                        showToast('Fehler beim Importieren: Die Datei konnte nicht gelesen werden.', { type: 'error', duration: 8000 });
                    }
                    return;
                }
    ...
    const markInvoiceExported = (proj, kind) => {
        ...
        setProjects(prev => prev.map(p => p.id === proj.id
            ? { ...p, invoiceStatus: 'exportiert', invoiceExportedAt: exportedAt } : p));
    ...
    const buildInvoiceHtml = (proj, laborLines, costLines, total, summaryMode) => {
        ...
        let html = '<p>Guten Tag,</p>'
            + `<p>anbei die Kostenaufstellung f&uuml;r Projekt &quot;${escapeHtml(proj.name)}&quot;.</p>`;

        const detailRows = [
            ['Projekt', proj.name],
            ['Projektnummer', proj.projectNumber || '-'],
            proj.address ? ['Adresse', proj.address] : null,
            (cc && cc !== '/') ? ['Land', cc] : null,
            proj.projType ? ['Typ', proj.projType] : null,
            (proj.size != null && proj.size !== '') ? ['Gr\u00f6\u00dfe', proj.size] : null,
            proj.ibnWeek ? ['IBN-Woche', proj.ibnWeek] : null,
            ['Datum', new Date().toLocaleDateString('de-DE')],
        ].filter(Boolean);
        html += '<table style="border-collapse:collapse;border:1px solid #94a3b8;margin-bottom:12px;">'
            + detailRows.map(([k, v]) => `<tr><td style="${th}">${escapeHtml(k)}</td><td style="${td}">${escapeHtml(v)}</td></tr>`).join('')
            + '</table>';
        ...
        if (includedLabor.length > 0) {
            let laborTotal = 0;
            includedLabor.forEach(l => { laborTotal += l.cost; });
            html += '<h4 style="margin:14px 0 4px;">Personalkosten</h4>';
            if (summaryMode) {
                html += `<p>Summe: <strong>${fmt2(laborTotal)} EUR</strong></p>`;
            } else {
                const rowsHtml = includedLabor.map(l =>
                    `<tr><td style="${td}">${escapeHtml(l.emp?.name || 'Unbekannt')}</td>`
                    + `<td style="${tdNum}">${l.hours}</td>`
                    + `<td style="${tdNum}">${fmt2(l.rate)}</td>`
                    + `<td style="${tdNum}">${fmt2(l.cost)}</td></tr>`
                ).join('');
                html += '<table style="border-collapse:collapse;border:1px solid #94a3b8;">'
                    + `<thead><tr><th style="${th}">Mitarbeiter</th><th style="${th}">Std.</th><th style="${th}">EUR/h</th><th style="${th}">Betrag (EUR)</th></tr></thead>`
                    + `<tbody>${rowsHtml}</tbody>`
                    + `<tfoot><tr><td style="${th}" colspan="3">Summe</td><td style="${th}text-align:right;">${fmt2(laborTotal)}</td></tr></tfoot>`
                    + '</table>';
            }
        }
        ...
        if (includedCosts.length > 0) {
            ...
            includedCosts.forEach(({ ci, emp }) => {
                (ci.lines || []).forEach(l => {
                    ...
                    sumsByType[l.type] = (sumsByType[l.type] || 0) + amt;
                    const desc = [ci.description, l.comment].filter(Boolean).join(' - ');
                    detailRowsHtml.push(`<tr><td style="${td}">${escapeHtml(emp?.name || 'Unbekannt')}</td>`
                        + `<td style="${td}">${escapeHtml(cfg.invoiceLabel)}</td>`
                        + `<td style="${td}">${escapeHtml(desc || '-')}</td>`
                        + `<td style="${tdNum}">${fmt2(amt)}</td></tr>`);
                });
            });
            html += '<h4 style="margin:14px 0 4px;">Zusatzkosten</h4>'
                + '<table style="border-collapse:collapse;border:1px solid #94a3b8;">';
            if (summaryMode) {
                const sumRowsHtml = COST_LINE_TYPE_ORDER.filter(type => sumsByType[type] > 0)
                    .map(type => `<tr><td style="${td}">${escapeHtml(COST_LINE_TYPES[type].invoiceLabel)}</td>`
                        + `<td style="${tdNum}">${fmt2(sumsByType[type])}</td></tr>`).join('');
                html += `<thead><tr><th style="${th}">Kategorie</th><th style="${th}">Betrag (EUR)</th></tr></thead>`
                    + `<tbody>${sumRowsHtml}</tbody>`
                    + `<tfoot><tr><td style="${th}">Summe</td><td style="${th}text-align:right;">${fmt2(costsTotal)}</td></tr></tfoot>`;
            } else {
                html += `<thead><tr><th style="${th}">Mitarbeiter</th><th style="${th}">Kategorie</th><th style="${th}">Beschreibung</th><th style="${th}">Betrag (EUR)</th></tr></thead>`
                    + `<tbody>${detailRowsHtml.join('')}</tbody>`
                    + `<tfoot><tr><td style="${th}" colspan="3">Summe</td><td style="${th}text-align:right;">${fmt2(costsTotal)}</td></tr></tfoot>`;
            }
            html += '</table>';
        }

        html += `<p style="margin-top:14px;"><strong>GESAMT NETTO: ${fmt2(total)} EUR</strong></p>`
    ...
    const copyInvoiceTable = () => {
        ...
        const { subject, body } = buildInvoiceEmailText(proj, laborLines, costLines, total, summaryMode);
        const { html } = buildInvoiceHtml(proj, laborLines, costLines, total, summaryMode);
        const text = `${subject}\n\n${body}`;
        const done = () => showToast(t('invoice.copiedTable'), { type: 'success', duration: 3000 });
        const fail = () => showToast(t('invoice.copyFailed'), { type: 'error' });
    ...
    const handleInvoiceSendEmail = () => {
        ...
        if (!proj) return;
        const { laborLines, costLines, total } = buildInvoiceData(proj, invoiceSelection);
        const { subject, body } = buildInvoiceEmailText(proj, laborLines, costLines, total, invoiceEmailMode === 'summary');
    ...
    const ProjFormModal = () => {
        ...
        const duplicateName = !nameMissing && projects.some(p =>
            p.id !== editingProjectId
            && p.name.trim().toLowerCase() === projForm.name.trim().toLowerCase());
        const weekOrderInvalid = !!(projForm.startWeek && projForm.ibnWeek
            && compareWeekIds(projForm.ibnWeek, projForm.startWeek) < 0);
        ...
        const sectionTitle = (txt) => (
            <h4 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider pt-1">{txt}</h4>
        );
        const save = () => {
            if (!canSave) return;
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="flex gap-2 items-stretch">
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <span className={`px-3 py-2 rounded-md text-sm font-mono font-bold border min-w-[3.5rem] text-center flex items-center justify-center ${styled}`} title="Auflösung des Eingabefelds">
                                                {code}
                                            </span>
                                        );
                                    })()}
                ...
                <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                    ...
                    <div className="p-6 bg-slate-50 border-t border-slate-100 space-y-3">
                        <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-xs text-amber-800">
                            ...
                            <span>{t('invoice.copyPasteHint')}</span>
                        </div>
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={copyInvoiceTable}
                                    ...
                                    <IconCopy size={14}/> {t('invoice.copyBtn')}
                                </button>
                                {invoiceRecipient && (
