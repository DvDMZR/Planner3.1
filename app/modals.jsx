    ...
    const handleSave = () => {
        ...
        if (newTotal > 100) {
            ...
            requestConfirm({
                ...
                message: t('modal.overutilWarning', { empName, pct: Math.round(newTotal) }),
                        ...
                        <div>
                            ...
                            {formData.type === 'project' && (
                                <select value={formData.reference} onChange={e => setFormData({...formData, reference: e.target.value})} className="w-full p-2 border border-slate-300 rounded-md text-sm">
                                    {projects
                                        .filter(p => ['active','planned'].includes(computeAutoStatus(p)))
                                        .sort((a, b) => {
...
const CostItemModal = ({
    ...
    const [settleStatus, setSettleStatus] = useState(
        existingItem ? getSettlementStatus(existingItem) : (projectId ? 'to_submit' : 'remain_on_kst'));
    ...
    const lineAmount = (l) => l.type === 'hours'
        ? safeNum(l.hours) * safeNum(l.hourlyRate)
        : safeNum(l.amount);
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="p-6 space-y-5 overflow-y-auto">
                    ...
                    <div className="p-3 rounded-lg border border-slate-200 bg-slate-50 flex items-center gap-3 flex-wrap">
                        <span className="text-xs font-medium text-slate-600">{t('costitem.settlement')}:</span>
                        <select value={settleStatus} onChange={e => setSettleStatus(e.target.value)}
                            ...
                            {SETTLEMENT_STATUS_ORDER.map(k =>
                                <option key={k} value={k}>{t(`travel.status.${k}`)}</option>)}
                        </select>
                        <span className="text-xs text-slate-500 ml-1">{t('costitem.targetAccount')}:</span>
                        ...
                        <span className="text-[10px] text-slate-400">{t('costitem.settlementHint')}</span>
                    </div>
                </div>
                <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between">
                    {existingItem ? (
...
const LoginModal = ({ appUsers, onLogin, onClose, t = (k) => k }) => {
    ...
    const [failedAttempts, setFailedAttempts] = useState(() => {
        try { return parseInt(localStorage.getItem('plannerLoginFails') || '0', 10) || 0; }
        catch(e) { return 0; }
    });
    const [lockUntil, setLockUntil] = useState(() => {
        try { return parseInt(localStorage.getItem('plannerLoginLockUntil') || '0', 10) || 0; }
        catch(e) { return 0; }
    });
    ...
    const handleLogin = async () => {
        ...
        if (user.role === 'admin') {
            ...
            try { ok = await verifyAdminPin(pin, user); } catch(e) { ok = false; }
        ...
        if (!ok) {
            ...
            try { localStorage.setItem('plannerLoginFails', String(next)); } catch(e) {}
            if (next >= LOGIN_LOCK_THRESHOLD) {
                const until = Date.now() + LOGIN_LOCK_DURATION_MS;
                setLockUntil(until);
                setNow(Date.now());
                try { localStorage.setItem('plannerLoginLockUntil', String(until)); } catch(e) {}
                                ...
                                {dependents.costItems.slice(0, 50).map(c => {
                                    ...
                                    return (
                                        <li key={c.id} className="text-xs text-slate-700 flex justify-between gap-2">
                                            ...
                                            <span className="text-slate-500 shrink-0 tabular-nums">{(c.amount || 0).toFixed(2)} €</span>
                                        </li>
                                    );
                                })}
