...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (SP_CONTEXT) {
        ...
        try {
          ...
          try {
            ...
            if (pendingTs) {
              const spMetaTs = spFileTimestampsRef.current['meta.json'] ? new Date(spFileTimestampsRef.current['meta.json']).getTime() : 0;
              if (pendingTs > spMetaTs + 5000) {
                showToast('Vorsicht: Beim letzten Schließen konnten Änderungen möglicherweise nicht vollständig gespeichert werden. Bitte prüfe die Daten.', {
                  type: 'warning',
                  duration: 0
                });
              }
      ...
      if (parsedData) {
        ...
        if (parsedData.emailTemplate) setEmailTemplate(prev => ({
          ...prev,
  ...
  useEffect(() => {
    document.title = `Einsatzplanung 3.0 – ${APP_VERSION}`;
  }, []);
  ...
  useEffect(() => {
    ...
    localSaveTimer.current = setTimeout(() => {
      ...
      if (FS_MODE && dirHandleRef.current) {
        saveSplitState(stateDataFull, lastSavedFsRef.current, (filename, payload) => fsSaveFile(dirHandleRef.current, filename, payload)).then(async () => {
    ...
    if (SP_CONTEXT) {
      ...
      spSaveTimer.current = setTimeout(async () => {
        ...
        const runSave = async () => {
          ...
          const written = await saveSplitState(stateDataFull, lastSavedSpRef.current, (filename, payload) => spSaveFile(SP_CONTEXT, filename, payload, filename === 'meta.json' ? null : spFileEtagsRef.current[filename]));
        ...
        try {
          localStorage.setItem('plannerPendingWrite', String(Date.now()));
        } catch (e) {}
        try {
          await runSave();
  ...
  useEffect(() => {
    ...
    showToast(`Erster Start: Admin-PIN ist ${pin} – bitte sofort unter Verwaltung → Benutzer ändern!`, {
      type: 'warning',
      duration: 0
    });
  }, [appUsers, showToast]);
  ...
  const runBackup = useCallback(async (reason = 'auto') => {
    ...
    const pruneSp = async () => {
      ...
      try {
        ...
        const autos = files.filter(f => f.name?.startsWith('backup-')).sort((a, b) => (a.ts || '').localeCompare(b.ts || ''));
        const excess = autos.length - BACKUP_KEEP_COUNT;
        for (let i = 0; i < excess; i++) {
          await spDeleteBackup(SP_CONTEXT, autos[i].name).catch(() => {});
        }
      } catch (e) {
        console.warn('[BACKUP] SP prune failed', e);
      }
    };
    const pruneFs = async () => {
      if (reason !== 'auto') return;
      try {
        ...
        console.warn('[BACKUP] FS prune failed', e);
      }
    };
  ...
  const applyRemoteSnapshot = useCallback((data, {
    ...
    if (data.costItems) {
      setCostItems(prev => data.costItems.length > 0 || prev.length === 0 ? migrateCostItems(data.costItems) : prev);
            ...
            if (onlyAuditChanged) {
              ...
              if (Array.isArray(auditData?.auditLog)) {
                setAuditLog(prev => {
                  ...
                  lastSavedSpRef.current['audit.json'] = JSON.stringify({
                    auditLog: merged
                  });
                  return merged;
                });
  ...
  const commandNavItems = useMemo(() => {
    const items = [{
      ...
      icon: /*#__PURE__*/React.createElement(IconGanttChart, {
        size: 16
      }),
      onSelect: () => {
    ...
    if (currentUser) {
      items.push({
        id: 'nav-utilization',
        label: t('nav.utilization'),
        icon: /*#__PURE__*/React.createElement(IconBarChart, {
          size: 16
        }),
        ...
        id: 'nav-setup-proj',
        label: t('nav.projects') + ' (' + t('nav.section.admin') + ')',
        ...
        icon: /*#__PURE__*/React.createElement(IconTag, {
          size: 16
        }),
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, t('fs.noSync')))), isHelpModalOpen && HelpModal(), /*#__PURE__*/React.createElement(CommandPalette, {
    ...
    t: t
  }), isLoginModalOpen && /*#__PURE__*/React.createElement(LoginModal, {
    appUsers: appUsers,
    onLogin: loginUser,
